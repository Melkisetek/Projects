...
const estadoTareas = document.getElementById('estadoTareas');
...
function cargarDatosIniciales() {
    ...
    const tareaInicial = new listado_apuntes_1.listadoApuntes('Compra de la semana', 'Recoger la compra del supermercado');
    ...
    const tareaInicial2 = new listado_apuntes_1.listadoApuntes('Estudiar TypeScript', 'Repasar conceptos básicos de TypeScript');
...
function agregarNuevaTarea(e) {
    ...
    if (titulo && descripcion) {
        ...
        const nuevaTarea = new listado_apuntes_1.listadoApuntes(titulo, descripcion);
 ...
 */
function manejarEventosTareas(e) {
    const target = e.target;
    const accion = target.dataset.action || target.closest('[data-action]')?.dataset.action;
    if (!accion)
        return;
    // Busca el elemento padre que representa la tarea
    const tareaElemento = target.closest('.tarea');
    if (!tareaElemento)
        return;
    ...
    switch (accion) {
        ...
        case 'modificar':
            ...
            break;
        case 'completar':
            toggleCompletarTarea(index, target.checked);
            break;
    }
}
/**
 ...
 */
function manejarCambiosTareas(e) {
    const target = e.target;
 ...
 */
function toggleCompletarTarea(index, completada) {
    if (index >= 0 && index < tareas.length) {
        // Actualiza el estado de la tarea
        tareas[index].completada = completada;
        // Actualiza la clase CSS del elemento
        const tareaElemento = document.querySelector(`.tarea[data-index="${index}"]`);
        if (tareaElemento) {
            if (completada) {
                tareaElemento.classList.add('completada');
            }
...
function cargarTareasDesdeLocalStorage() {
    try {
        ...
        if (tareasGuardadas) {
            ...
            listado_apuntes_1.listadoApuntes.contadorListas = 0;
            // Convierte los objetos planos a instancias de listadoApuntes
            tareas = tareasParseadas.map((t) => {
                const tarea = new listado_apuntes_1.listadoApuntes(t.titulo, t.descripcion);
                ...
                if (t.id > listado_apuntes_1.listadoApuntes.contadorListas) {
                    listado_apuntes_1.listadoApuntes.contadorListas = t.id;
                }
                return tarea;
            });
        }
